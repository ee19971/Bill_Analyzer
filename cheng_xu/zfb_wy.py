...
# 提取最小支出和最小收入的时间
min_expense_day = zfb.loc[zfb["支出金额（-元）"] == min_expense, "发生时间"].dt.strftime("%m月%d日").iloc[0]   # 最小支出的时间
max_expense_day = zfb.loc[zfb["支出金额（-元）"] == max_expense, "发生时间"].dt.strftime("%m月%d日").iloc[0]  # 最大支出的时间
