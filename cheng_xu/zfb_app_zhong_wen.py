...
def zfb_app_zhong_wen_csv(file_path: str)->str:
    # Section 常量
    spike = search_file_line(file_path, '交易时间,')[0]['line'] - 1
    ji_lu = search_file_line(file_path, '笔记录')[0]['content']
    shou_ru = search_file_line(file_path, '收入：')[0]['content']
    zhi_chu = search_file_line(file_path, '支出：')[0]['content']
    bu_ji_shou_zhi = search_file_line(file_path, '不计收支：')[0]['content']

    # Section 读取文件
    zfb_app = pd.read_csv(file_path, skiprows=spike, encoding='GB18030')
    ...
    # 按月份统计支出和收入
    zfb_app["交易月"] = zfb_app["交易时间"].dt.to_period("M")  # 提取交易时间的年月信息
    monthly_summary = (
        zfb_app.groupby("交易月").agg(
            总支出=("支出金额", "sum"),
            总收入=("收入金额", "sum"))
        .reset_index())
    # 找到支出最多的月份
